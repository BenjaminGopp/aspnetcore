...
/// </summary>
internal sealed class HybridClientHandler : HttpMessageHandler
{
    ...
    internal HybridClientHandler(PathString pathBase, ApplicationWrapper application, Action<HttpContext>? additionalContextConfiguration = null)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var contextBuilder = new HybridHttpContextBuilder(_application, AllowSynchronousIO, PreserveExecutionContext);
        ...
        contextBuilder.Configure((context, _) =>
        {
            if (request.Headers.TryGetValues("aspnetcore-webviewname", out var webviewname))
            {
                request.Headers.Remove("aspnetcore-webviewname");
            }

            if (request.Headers.TryGetValues("aspnetcore-origin", out var origin))
            {
                request.Headers.Remove("aspnetcore-origin");
            }

            context.Features.Set<IHttpRequestMessageFeature>(new HttpRequestMessageFeature() { RequestUri = request.RequestUri });
            context.Features.Set<IWebViewFeature>(new WebViewFeature() { WebViewName = webviewname?.First(), Origin = origin?.First()});
        ...
        //BGO
        if (httpContext.Items.TryGetValue("__RequestUnhandled", out _))
        {
            response.Headers.TryAddWithoutValidation("__RequestUnhandled", "");
        }
